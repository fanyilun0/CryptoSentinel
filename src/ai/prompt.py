...
**数据说明**：
- 数据按时间顺序从旧到新排列，最后一条是最新数据
- `price`: BTC价格（美元）
- `mvrv`: MVRV比率（Market Value to Realized Value），即比特币市场市值与已实现市值的比率。MVRV反映的是全网持有者的平均未实现盈亏水平，可作为市场估值的参考维度之一
- `fear_greed_value`: 恐惧贪婪指数（0-100）

**分析原则**：
- 每个指标仅代表市场的一个观察角度，需要结合多个维度综合研判
- 指标的参考价值会随市场结构变化而变化，避免机械套用固定阈值
- 重点关注指标的趋势变化和多指标间的背离/共振信号

请按照以下结构提供全面分析和精确建议：

## 一、市场周期分析
（150字以内）
[分析当前市场周期位置（积累期/上升期/分配期/下跌期），关键支撑阻力位，结合MVRV比率和恐惧贪婪指数等多维度综合研判市场情绪与估值状态。明确判断当前处于牛市还是熊市阶段。仅使用提供的数据进行分析。]

## 二、技术指标解读
1. 价格趋势：[分析BTC近期价格趋势、关键价格位]
2. 市场估值：[结合MVRV比率的趋势变化和当前水平，综合恐惧贪婪指数，分析市场估值状态与情绪]
3. 周期阶段：[综合以上指标判断当前所处的市场周期阶段，是否存在多指标间的背离或共振信号]
