...
重要提示：
1. 必须在开头提供TL;DR摘要，明确牛熊状态和决策关键字
2. 必须先提供详细的六个部分分析内容，然后才提供JSON格式的决策数据
3. 仓位调整必须使用标准关键字：加仓/减仓/满仓/清仓/持有
4. 所有金额计算基于总投资预算${total_budget:.0f}
5. 提供具体的利润计算，包括美元金额和百分比
6. 所有建议必须基于已提供的数据，不要引用未提供的指标或信息
7. 给出具体可执行的策略而非模糊建议
8. 考虑当前持仓状态和成本基础做出建议
9. 提供明确的数值和百分比，避免模糊表述
10. 确保每个风险因素都有相应的应对策略
11. 所有历史数据比较需具体说明数值和时间点
"""
